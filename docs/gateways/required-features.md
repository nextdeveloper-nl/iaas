...
| R5 | A newly provisioned gateway is reachable with real, working credentials the moment it's ready — no manual pfSense setup wizard, and no inbound connectivity to the appliance required. | ✅ Implemented | `Jobs\Gateways\CollectGatewayCredentials` (delayed, retrying job) + `PfSenseGatewayDriver::bootstrap()`, populating `ssh_username`/`ssh_password`/`ip_addr` on the `Gateways` row by sending `pfsense.set_password` to the VM's `pfsense.agent` over NATS (the agent rotates the password locally - no SSH/inbound port needed, unlike the original implementation), then confirming readiness via a NATS `system.info` call (see R6-R8; `api_token`/`api_url` are no longer used - firewall/NAT/health/bootstrap all go over NATS, not a REST API or SSH on the box). |
