...
Status: **Phase 0 closed out.** Scaffold fixes, `XenServer82SshDriver`, event pipeline
normalization, and every live Action/Job/Controller call site are migrated onto
`VirtualMachineManager`. Phases 1-4 (XCP-ng/Proxmox/VMware/KVM) not started yet.

Two items were deliberately left as-is rather than migrated: `VirtualDiskImages/Copy.php`
(no underlying disk-copy operation exists anywhere to wrap) and
`Jobs/GarbageCollectors/CollectGarbageNetworks.php` (explicitly marked deprecated in its
own `handle()` via `trigger_deprecation()`, not a live call site worth migrating).
`EventsXenService::store()`'s webhook ingestion also stays XenServer-specific by design -
it parses XenAPI message-class events (`VM_STARTED`/`VM_STOPPED`), a different wire format
from the vm-class events the event translator handles, and message-class events are
explicitly out of normalization scope per §5 below; its `dd()`-on-unhandled-event crash
bug was fixed without touching that scope decision. See §6 for the full closeout list.
...
**Phase 0 — Fix the scaffold, wire XenServer through it. (Closed out.)**

Landed: scaffold bug fixes, all 8 capability interfaces (Snapshot/Clone/Resize/Disk/Network/HostSync/Console/EventTranslator/Export/ConfigurationIso/Provisioning/Backup), `XenServer82SshDriver` implementing all of them, `config/virtualization.php` + service-provider wiring, `NormalizedHypervisorEvent` + `XenServerEventTranslator`, `ComputeComputeMemberEventsJob::vmModOperation()` rewired, every VM Action (power-state, Snapshot, Sync, Commit, HealthCheck, ConvertToTemplate, Export, ExportAsMachineImage, Delete, MountCd/EjectCd, UpdateConfigurationIso, Backup), every ComputeMembers Action (Initiate/Scan/ScanVirtualMachines/UpdateResources/UpdateStorageVolumes/CheckServices/MountVmRepo/UnmountVmRepo/MountIsoRepo), VirtualDiskImages (Destroy/Detach/Resize/Create/Attach/Sync), VirtualNetworkCards (Attach/Detach), StorageVolumes/Scan, ComputePools/ImportVirtualMachine (plus 3 real bugs fixed along the way: a missing 4th constructor arg, a VBD-uuid-vs-SR-uuid storage lookup mismatch, a `??`/`?:` typo), BackupJobs (RunBackupJob/InitiateMultilevelBackupJob, plus two `dd()` crash bugs fixed), `Jobs/VirtualMachines/Fix.php`/`GenerateCloudInitImage.php`, `VirtualMachinesConsoleController`'s session-ref console flow (via a new `VirtualMachinesService::getConsoleSession()` wrapper), `hypervisor_model` branching consolidated in `StorageVolumes/Scan.php`.

**Deliberately not migrated** (not gaps — each has a specific reason, not "not gotten to yet"):
...
- `Jobs/GarbageCollectors/CollectGarbageNetworks.php` — its own `handle()` calls `trigger_deprecation(...)` and is flagged for removal; not a live call site worth migrating.
- `EventsXenService::store()` (backing `ComputeMemberEventsController`) — parses XenAPI message-class events (`VM_STARTED`/`VM_STOPPED`), a different wire format from the vm-class events the translator handles; message-class events are explicitly out of normalization scope per §5. Its `dd()`-on-unhandled-event crash was fixed as a straight bug fix, independent of this scope decision.
