...
/**
 * Step-by-step VM migration command for testing and manual operation.
 *
 * Usage examples:
 *
 *   # 1. Propose an evacuation plan (dry-run, no DB changes)
 *   php artisan leo:migrate-vm --step=propose --vm-id=<vm-uuid> --target-id=<compute-member-uuid>
 *   php artisan leo:migrate-vm --step=propose --vm-id=<vm-uuid> --target-id=<cm-uuid> --storage-type=ssd
 *
 *   # 2. Approve the plan and create a migration record
 *   php artisan leo:migrate-vm --step=approve --vm-id=<vm-uuid> --target-id=<cm-uuid>
 *
 *   # 3-12. Run individual migration steps (use the migration UUID printed by approve)
 *   php artisan leo:migrate-vm --step=pre-flight       --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=collect-metadata --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=coalesce-vhd     --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=shutdown         --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=copy-vhd         --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=rescan-sr        --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=recreate-vm      --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=validate         --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=sync-db          --migration-id=<uuid>
 *   php artisan leo:migrate-vm --step=start-vm         --migration-id=<uuid>
 *
 *   # Run the full migration end-to-end
 *   php artisan leo:migrate-vm --step=run --migration-id=<uuid>
 */
class MigrateVirtualMachine extends Command
{
    protected $signature = 'leo:migrate-vm
    ...
    private function stepApprove(): void
    {
        ...
        $this->newLine();
        $this->line('Next step:');
        $this->line('  php artisan leo:migrate-vm --step=pre-flight --migration-id=' . $migration->uuid);
    }
    ...
    private function stepPreFlight(): void
    {
        ...
        $this->info('[Step 1] Running pre-flight checks...');
    ...
    private function stepCoalesceVhd(): void
    {
        ...
        // Allow force-delete-snapshots flag to be injected into options
    ...
    private function stepRun(): void
    {
        ...
        if (!$this->confirm(
            'Run the full migration end-to-end for migration ' . $migration->uuid . '?',
            false
        )) {
            $this->line('Aborted.');
            return;
    ...
    private function nextStepHint(string $step, string $migrationUuid): void
    {
        $this->newLine();
        $this->line('Next step:');
        $this->line('  php artisan leo:migrate-vm --step=' . $step . ' --migration-id=' . $migrationUuid);
    }
    ...
    private function printSteps(): void
    {
        $this->table(['Step', 'Description'], [
            ...
            ['pre-flight',       'Verify SSH, VM state, SR space, NFS mounts'],
            ...
            ['copy-vhd',         'Copy VHD files from source to target storage via rsync'],
