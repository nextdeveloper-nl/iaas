 ...
 */
class AbstractVirtualNetworkCardsService
{
    public static function get(VirtualNetworkCardsQueryFilter $filter = null, array $params = []) : Collection|LengthAwarePaginator
    {
        ...
        */
        if($filter == null) {
            $filter = new VirtualNetworkCardsQueryFilter(new Request());
        }
        ...
        $model = VirtualNetworkCards::filter($filter);
    ...
    {
        return VirtualNetworkCards::all();
    }
     ...
     * @return mixed
     */
    public static function getByRef($ref) : ?VirtualNetworkCards
    {
        return VirtualNetworkCards::findByRef($ref);
    }

    public static function getActions()
    {
        $model = VirtualNetworkCards::class;
    ...
    {
        $object = VirtualNetworkCards::where('uuid', $objectId)->first();

        $action = '\\NextDeveloper\\IAAS\\Actions\\VirtualNetworkCards\\' . Str::studly($action);
     ...
     *
     * @param  $id
     * @return VirtualNetworkCards|null
     */
    public static function getById($id) : ?VirtualNetworkCards
    {
        return VirtualNetworkCards::where('id', $id)->first();
    }
    ...
    {
        try {
            $obj = VirtualNetworkCards::where('uuid', $uuid)->first();
     ...
     */
    public static function updateRaw(array $data) : ?VirtualNetworkCards
    {
    ...
    {
        $model = VirtualNetworkCards::where('uuid', $id)->first();
